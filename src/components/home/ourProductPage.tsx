...
export default function OurProductPage() {
  return (
    <section
      id="products"
      className="pb-12 pt-0 md:pt-6 px-6 lg:px-12 bg-white overflow-hidden"
    >
      <div className="max-w-7xl mx-auto">
        <ScrollReveal className="mb-8 flex flex-col items-start text-left">
          <span className="inline-block bg-brand-primary text-white px-6 py-5 border-b-4 border-gray-300 rounded-full text-[12px] font-bold mb-6 shadow-sm">
          ...
          {productCards.map((card, index) => (
            ...
            >
              ...
              <div className="w-full flex flex-col flex-1">
                <h3 className="text-[16px] font-bold text-brand-primary mb-2">
                  {card.title}
                </h3>
                ...
                {card.title === "Gamification" ? (
                  <Link
                    href={`/book-demo?product=${encodeURIComponent(card.title)}`}
                    className="bg-brand-primary text-white text-[13px] font-semibold px-4 py-2.5 rounded-md hover:opacity-90 transition-colors mt-auto w-fit"
                  >
                    Book Demo
                  </Link>
                ) : (
